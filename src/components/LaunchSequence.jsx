    ...
    try {
      ...
      await updateDoc(doc(db, 'users', userId), {
        ...
        scoreDistribution: data.scoreDistribution || {
          excellent: data.leads.filter(l => l.score >= 85).length,
          good: data.leads.filter(l => l.score >= 70 && l.score < 85).length,
          moderate: data.leads.filter(l => l.score >= 50 && l.score < 70).length
        },
        tier: 'scout'
      });

      console.log('💾 Leads saved to Firebase');
