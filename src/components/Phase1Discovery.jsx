...
export default function Phase1Discovery({ scoutData, icpBrief, onComplete }) {
  ...
  const [validationHistory, setValidationHistory] = useState([]); // Track history for undo
  ...
  const handleReasonSubmit = async (reasons) => {
    ...
    const newReasons = { ...newValidationResults[reasonKey] };
    reasons.forEach(reason => {
      newReasons[reason] = (newReasons[reason] || 0) + 1;
    });
    newValidationResults[reasonKey] = newReasons;
    ...
    setValidationHistory(prev => [...prev, {
      ...
      reasons: reasons,
      resultsSnapshot: JSON.parse(JSON.stringify(validationResults))
    }]);

    setShowReasonModal(false);
    
    // Save progress immediately
    await saveProgressToFirebase(newValidationResults, currentCardIndex + 1);
  ...
  const handleUndo = () => {
    ...
    const lastEntry = validationHistory[validationHistory.length - 1];
    setValidationResults(lastEntry.resultsSnapshot);
    setCurrentCardIndex(lastEntry.cardIndex);
    setValidationHistory(prev => prev.slice(0, -1));
    ...
    try {
      ...
      await setDoc(doc(db, 'missions', user.uid, 'current', 'phase1'), {
        ...
        progress: {
          ...
          totalCards: validationSample.length,
          percentComplete: Math.round((cardIndex / validationSample.length) * 100)
        },
        lastUpdated: serverTimestamp()
      }, { merge: true });
      
      console.log(`💾 Progress saved: ${cardIndex}/${validationSample.length}`);
    } catch (err) {
  ...
  const showValidationSummary = async () => {
    ...
    try {
      const user = auth.currentUser;
      await updateDoc(doc(db, 'missions', user.uid, 'current', 'phase1'), {
