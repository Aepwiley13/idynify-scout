/**
 * HunterEngagementPanel — Stage-specific engagement context for Hunter-stage contacts.
 *
 * Goal: Book the meeting — active pursuit, get on the calendar.
 * Primary CTA: Move to Sniper (meeting booked)
 * Skip CTA:    Move straight to Basecamp (deal closed without formal Sniper phase)
 *
 * Note: Sits alongside the existing HunterActionPanel for now.
 */

import { useState } from 'react';
import { Crosshair, ChevronDown, ChevronUp, Loader } from 'lucide-react';
...
export default function HunterEngagementPanel({ contact, onMoved }) {
  const T = useT();
  ...
  async function handleMoveToSniper() {
    try {
      setLoading(true);
      setError(null);
      ...
      setDone(true);
      if (onMoved) onMoved({ stageTo: 'sniper', reason, note });
    } catch (err) {
      console.error('[HunterEngagementPanel] Move to Sniper failed:', err);
  ...
  async function handleSkipToBasecamp() {
    try {
      setLoading(true);
      setError(null);
      const user = getEffectiveUser();
      ...
      setDone(true);
      if (onMoved) onMoved({ stageTo: 'basecamp', reason: 'deal_won', note });
    } catch (err) {
      console.error('[HunterEngagementPanel] Skip to Basecamp failed:', err);
      setError('Failed to move contact. Please try again.');
    } finally {
      setLoading(false);
    }
    ...
    <div style={{
      ...
      >
        <div style={{
          ...
          <Crosshair size={15} color={STAGE_COLOR} />
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: T.text }}>Book the Meeting</div>
          <div style={{ fontSize: 11, color: T.textMuted, marginTop: 1 }}>
            Active pursuit — get on the calendar
          </div>
        </div>
        {expanded
      ...
      {expanded && (
        <div style={{ padding: '0 16px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ fontSize: 12, color: T.textMuted, fontStyle: 'italic', padding: '6px 10px', background: `${STAGE_COLOR}08`, borderRadius: 7, borderLeft: `3px solid ${STAGE_COLOR}40` }}>
            Each touch should move toward a booked call — make it easy to say yes.
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {REASON_OPTIONS.map(opt => (
                ...
                >
                  ...
                  <input type="radio" name="hunter-eng-reason" value={opt.value} checked={reason === opt.value} onChange={() => setReason(opt.value)} style={{ display: 'none' }} />
                  {opt.label}
                </label>
              ))}
            </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="e.g. Meeting booked for next Tuesday, they want to see the product…"
              rows={2}
              style={{
          ...
          {/* Primary CTA */}
          <button
            onClick={handleMoveToSniper}
            disabled={loading}
            style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7,
              padding: '10px 18px', borderRadius: 9,
              background: loading ? `${STAGE_COLOR}60` : `linear-gradient(135deg, ${STAGE_COLOR}, #5b21b6)`,
              border: 'none', color: '#fff', fontSize: 13, fontWeight: 700,
              cursor: loading ? 'not-allowed' : 'pointer', transition: 'opacity 0.15s',
            }}
          >
            {loading
              ? <><Loader size={14} style={{ animation: 'hunterEngSpin 1s linear infinite' }} />Moving…</>
          ...
          {!loading && (
            ...
            >
              Deal already closed? → Move straight to Basecamp
            </button>
          )}

          <style>{`@keyframes hunterEngSpin { to { transform: rotate(360deg); } }`}</style>
        </div>
      )}
    </div>
  );
