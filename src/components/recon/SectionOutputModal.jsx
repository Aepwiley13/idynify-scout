...
export default function SectionOutputModal({ section, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-gradient-to-br from-gray-900 to-black border-2 border-cyan-500/30 rounded-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="bg-black/60 border-b border-cyan-500/30 p-6 flex items-center justify-between">
          <div>
        ...
        {/* Tabs */}
        <div className="bg-black/40 border-b border-cyan-500/20 px-6 flex gap-2">
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {activeTab === 'raw' && (
            <div className="space-y-4">
              <h3 className="text-xl font-bold text-white mb-4 font-mono">Your Input Data</h3>
              {Object.entries(rawAnswers).map(([key, value]) => (
                <div key={key} className="bg-black/40 rounded-lg p-4 border border-cyan-500/20">
                  <p className="text-sm text-gray-400 mb-2 font-mono uppercase">{formatFieldName(key)}</p>
        ...
        {/* Footer */}
        <div className="bg-black/60 border-t border-cyan-500/30 p-4 flex justify-between items-center">
          ...
          <button
            onClick={onClose}
            className="bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 px-6 py-2 rounded-lg border border-cyan-500/30 font-mono font-bold transition-all"
          >
            CLOSE
          </button>
        </div>
    ...
    return (
      <>
        ...
        <div className="bg-gradient-to-br from-cyan-900/20 to-blue-900/20 rounded-lg p-6 border border-cyan-500/30">
          <h4 className="text-lg font-bold text-cyan-400 mb-3 font-mono">👥 IDEAL CUSTOMER AT A GLANCE</h4>
          <p className="text-white text-lg leading-relaxed">{summary.idealCustomerGlance}</p>
        </div>

        <div className="bg-gradient-to-br from-green-900/20 to-emerald-900/20 rounded-lg p-6 border border-green-500/30">
          <h4 className="text-lg font-bold text-green-400 mb-3 font-mono">✅ PERFECT FIT INDICATORS</h4>
        ...
        <div className="bg-gradient-to-br from-red-900/20 to-orange-900/20 rounded-lg p-6 border border-red-500/30">
          <h4 className="text-lg font-bold text-red-400 mb-3 font-mono">🚫 ANTI-PROFILE (AVOID THESE)</h4>
          <ul className="space-y-2">
        ...
        <div className="bg-gradient-to-br from-purple-900/20 to-pink-900/20 rounded-lg p-6 border border-purple-500/30">
          <h4 className="text-lg font-bold text-purple-400 mb-3 font-mono">💡 KEY INSIGHT</h4>
          <p className="text-white text-lg leading-relaxed">{summary.keyInsight}</p>
        </div>
      </>
    );
  }
...
function SummaryCard({ title, data }) {
  ...
  return (
    <div className="bg-black/40 rounded-lg p-6 border border-cyan-500/20">
      <h4 className="text-lg font-bold text-cyan-400 mb-3 font-mono">{title}</h4>

      {typeof data === 'string' ? (
        <p className="text-white leading-relaxed">{data}</p>
      ) : typeof data === 'object' && !Array.isArray(data) ? (
        <div className="space-y-3">
          {Object.entries(data).map(([key, value]) => (
            <div key={key}>
              <p className="text-sm text-gray-400 mb-1 font-mono">{formatFieldName(key)}</p>
        ...
        <p className="text-white">{String(data)}</p>
      )}
    </div>
  );
}
