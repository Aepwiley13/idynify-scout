...
export default function FindContact({ onContactAdded, onCancel, initialSearchParams }) {
  ...
  const handleConfirmAndSave = async () => {
    ...
    try {
      ...
      // Identity resolution replaces the two hand-rolled checks that were
      // here: an exact document-id lookup and a case-sensitive email query.
      // Both were hierarchy steps 1 and 2 done partially; this runs all six.
  ...
  const ensureCompanyExists = async (contact, userId) => {
    ...
    const match = await resolveCompany(userId, { apollo_organization_id: apolloOrgId, name: companyName },
      { source: 'FindContact.ensureCompanyExists' });
      ...
      <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-gray-900 mb-1">Barry will help you find the right contact</p>
            ...
            <div className="space-y-4">
              <div className="bg-green-50 rounded-xl p-4 border border-green-200">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-gray-900 mb-1">Barry's Recommendation</p>
              ...
              {searchResults.length > 1 && (
                <div className="mt-4">
                  <p className="text-sm font-semibold text-gray-700 mb-2">Other potential matches:</p>
                  <div className="space-y-2">
                    {searchResults.slice(1).map((contact, idx) => (
                      <button
                        key={idx}
                        onClick={() => setSelectedContact(contact)}
                        className="w-full text-left p-3 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-lg transition-all"
                      >
                        <p className="font-semibold text-gray-900">{contact.name}</p>
                        <p className="text-sm text-gray-600">{contact.title} at {contact.organization_name}</p>
                      </button>
                    ))}
                  </div>
                </div>
  ...
  return (
    <div className="bg-white border-2 border-blue-400 rounded-xl p-6 shadow-lg">
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${confidenceColors[confidence] || confidenceColors.medium}`}>
