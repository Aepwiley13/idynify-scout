...
// ============================================================================
// ENGAGEMENT SEQUENCING - Schema Documentation (Step 5)
// ============================================================================

/**
 * Engagement Sequence Plan
 * Stored in users/{userId}/missions/{missionId}.sequence
 *
 * Generated by barryGenerateMissionSequence (Step 4 → Step 5 upgrade).
 * This is the PLAN — a 2-4 step strategic structure.
 * Actual message content is generated just-in-time via barryGenerateSequenceStep.
 *
 * Schema:
 * {
 *   steps: [
 ...
 *       reasoning: string,             // Why Barry recommends this step
 *       suggestedTiming: string,       // e.g., 'Day 0', 'Day 4'
 *       suggestedDayOffset: number,    // Numeric day offset from start
 *       approvalRequired: true         // ALWAYS true — guardrail
 *     }
 *   ],
 *   sequenceRationale: string,         // Why this sequence structure makes sense
...
/**
 * Per-Contact Sequence State
 * Stored in users/{userId}/missions/{missionId}.contacts[].{sequenceFields}
 *
 * Each contact in a mission progresses through the sequence independently.
 * The sequence plan is per-mission; the progression is per-contact.
 *
 * Schema (fields on each contact entry in mission.contacts[]):
 * {
 *   currentStepIndex: number,          // 0-based, which step we're on
 *   sequenceStatus: 'pending' | 'active' | 'awaiting_outcome' | 'completed',
 *   lastOutcome: 'no_reply' | 'replied_positive' | 'replied_negative' | 'not_sure' | null,
 *   stepHistory: [
 *     {
 ...
 *       generatedContent: {            // Content generated by barryGenerateSequenceStep
 *         subject: string | null,
 *         body: string,
 *         channel: string,
 ...
 *       outcomeRecordedAt: string | null  // ISO timestamp
 *     }
 *   ]
 * }
 *
 * Lifecycle:
 *   pending → active (first step proposed)
 *   active → awaiting_outcome (step sent, waiting for user to record outcome)
 *   awaiting_outcome → active (outcome recorded, next step ready)
 *   active/awaiting_outcome → completed (all steps done or sequence_complete trigger)
 */

/**
 * Sequence Timeline Events
 * Stored in users/{userId}/contacts/{contactId}/timeline/{eventId}
 *
 * Five new event types added to the Engagement Timeline:
 *
 * sequence_step_proposed:
 *   actor: 'barry'
 *   metadata: { missionId, stepIndex, stepType, channel, suggestedTiming }
 *
 * sequence_step_approved:
 *   actor: 'user'
 *   metadata: { missionId, stepIndex, channel }
 *
 * sequence_step_sent:
 *   actor: 'user'
...
/**
 * Contact State Machine — Step 5 Additions
 *
 * New status: 'In Conversation'
 *   Triggered by: positive_reply (user records positive outcome)
 *   Meaning: Dialogue is active, mission is progressing
 *   Priority: 5 (between Awaiting Reply at 4 and Mission Complete at 6)
 *
 * New triggers:
 *   positive_reply → 'In Conversation' (always applies)
 *   sequence_complete → 'Mission Complete' (always applies)
 *
 * Updated status flow:
 *   New → Engaged → In Campaign → Active Mission → Awaiting Reply →
 *   In Conversation → Mission Complete → Dormant
 *
 * See: src/utils/contactStateMachine.js
 */
