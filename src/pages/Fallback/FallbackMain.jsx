/**
 * FallbackMain.jsx — Two-column nav shell for the FALLBACK module.
 *
 * Architecture:
 *  ┌─────────────────────────────────────────────────────────┐
 ...
 *  └─────────────────────────────────────────────────────────┘
 *
 * FALLBACK = Archive hub. People and companies you've archived,
 * lost, or moved out of active pipeline.
 *
 * This component is self-contained — it does NOT use MainLayout.
 * The App.jsx /fallback route must NOT use withLayout={true}.
 */
...
const MODULE_RAIL = [
  ...
  { id: 'fallback',       label: 'FALLBACK',       Icon: Archive,   route: null               }, // active module
];
...
// ─── FallbackShellInner ──────────────────────────────────────────────────────
function FallbackShellInner({ user }) {
  const T = useT();
  ...
  const userInitials = (user?.email || 'FB').slice(0, 2).toUpperCase();
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div
          ...
          style={{
            ...
            boxShadow: `0 4px 18px ${FALLBACK_AMBER}50`, flexShrink: 0, overflow: 'hidden',
            cursor: 'pointer', transition: 'transform 0.15s, box-shadow 0.15s',
          }}
          onMouseEnter={e => { e.currentTarget.style.transform = 'scale(1.08)'; e.currentTarget.style.boxShadow = `0 6px 22px ${FALLBACK_AMBER}70`; }}
          onMouseLeave={e => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.boxShadow = `0 4px 18px ${FALLBACK_AMBER}50`; }}
        >
          <img
            src={ASSETS.logoMark}
            alt="Mission Control"
          ...
          return (
            <div
              ...
              style={{
                ...
                cursor: 'pointer',
                background: active ? `${FALLBACK_AMBER}18` : 'transparent',
                border: `1px solid ${active ? FALLBACK_AMBER + '50' : 'transparent'}`,
                gap: 1, transition: 'all 0.15s', marginBottom: 2,
              }}
              onMouseEnter={e => { if (!active) e.currentTarget.style.background = T.surface; }}
              onMouseLeave={e => { if (!active) e.currentTarget.style.background = 'transparent'; }}
            >
              <mod.Icon size={14} color={active ? FALLBACK_AMBER : T.textFaint} />
              <span style={{ fontSize: 7, letterSpacing: 0, color: active ? FALLBACK_AMBER : T.textFaint, marginTop: 2, textAlign: 'center', width: '100%', lineHeight: 1.3 }}>
                {mod.label}
              </span>
            </div>
          );
        ...
        <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 7, alignItems: 'center' }}>
          <div
            ...
            onMouseEnter={e => { e.currentTarget.style.background = T.surface; e.currentTarget.style.border = `1px solid ${FALLBACK_AMBER}40`; }}
      ...
      <div style={{
        ...
        <div style={{ width: 190, display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontSize: 9, letterSpacing: 2, color: FALLBACK_AMBER, fontWeight: 700, marginBottom: 1 }}>
                FALLBACK
              </div>
              <div style={{ fontSize: 9, color: T.textFaint }}>Archive & lost records</div>
            </div>
            <div
              onClick={() => { setSubNavOpen(false); localStorage.setItem('fallback_subnav_collapsed', 'true'); }}
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    background: active ? `${FALLBACK_AMBER}12` : 'transparent',
                    borderLeft: `2px solid ${active ? FALLBACK_AMBER : 'transparent'}`,
                    transition: 'all 0.12s',
                  }}
                  onMouseEnter={e => { if (!active) e.currentTarget.style.background = T.surface; }}
                  onMouseLeave={e => { if (!active) e.currentTarget.style.background = 'transparent'; }}
                >
                  <it.Icon size={13} color={active ? FALLBACK_AMBER : T.textFaint} style={{ flexShrink: 0 }} />
          ...
          <div style={{
            ...
            <Av initials={userInitials} color={FALLBACK_AMBER} size={24} />
      ...
      {!subNavOpen && (
        <div
          onClick={() => { setSubNavOpen(true); localStorage.setItem('fallback_subnav_collapsed', 'false'); }}
...
export default function FallbackMain() {
  ...
  return <FallbackShellInner user={user} />;
}
