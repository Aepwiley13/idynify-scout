...
export default function SavedCompanies({ onSelectCompany }) {
  ...
  const noContactCompanies = companies.filter(c => c.contact_count === 0);
...
function SwipeDeck({ companies, totalActive, T, onFindContact }) {
  ...
  const showSkipOverlay = dragOffset.x < -45;
  const showFindOverlay = dragOffset.x > 45;
  ...
  if (remaining === 0) {
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 24px', gap: 18, textAlign: 'center' }}>
        ...
        <h2 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: T.text }}>You crushed it!</h2>
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '16px 22px 12px', flexShrink: 0 }}>
        ...
        <p style={{ margin: '0 0 10px', fontSize: 12, color: T.textFaint }}>{motivation.sub}</p>
        {/* Progress bar */}
      ...
      {/* ── Card deck area ── */}
      <div
        style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', padding: '0 22px 20px' }}
      >
        ...
        <div
          ...
          style={{
            ...
            transform: `translateX(${dragOffset.x}px) translateY(${dragOffset.y * 0.06}px) rotate(${rotation}deg)`,
            opacity,
            transition: isDragging ? 'none' : 'transform 0.32s cubic-bezier(0.34,1.56,0.64,1), opacity 0.3s',
            userSelect: 'none',
        ...
        >
          ...
          <div style={{ padding: '20px 20px 14px', flex: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 13, marginBottom: 16 }}>
              <div style={{
                ...
                background: T.surface, border: `1px solid ${T.border2 || T.border}`,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {current.location && (
                  <div style={{ fontSize: 11, color: T.textFaint, marginTop: 1 }}>{current.location}</div>
                )}
              </div>
              {current.fit_score > 0 && (
                <div style={{
                  flexShrink: 0, textAlign: 'center',
                  background: T.cyanBg || `${BRAND.cyan}15`, border: `1px solid ${T.cyanBdr || `${BRAND.cyan}30`}`,
            ...
            {current.description && (
              <p style={{
                margin: '0 0 13px', fontSize: 11, color: T.textMuted, lineHeight: 1.6,
            ...
            {(current.website_url || current.linkedin_url) && (
              <div style={{ display: 'flex', gap: 7 }}>
                {current.website_url && (
                  <button
                    onClick={e => { e.stopPropagation(); window.open(current.website_url, '_blank'); }}
                ...
                {current.linkedin_url && (
                  <button
                    onClick={e => { e.stopPropagation(); window.open(current.linkedin_url, '_blank'); }}
          ...
          {/* Action buttons */}
          <div style={{
            padding: '12px 18px 18px', borderTop: `1px solid ${T.border}`,
            display: 'flex', gap: 10,
          }}>
            <button
            ...
            <button
              ...
              style={{
                ...
                border: 'none', background: `linear-gradient(135deg, ${BRAND.pink}, #c0146a)`,
                color: '#fff', fontSize: 13, fontWeight: 700,
                cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                boxShadow: `0 4px 20px ${BRAND.pink}50`,
                transition: 'all 0.15s',
              }}
        ...
        <p style={{ marginTop: 14, fontSize: 10, color: T.textFaint, textAlign: 'center', flexShrink: 0 }}>
          Swipe right to find a contact · Swipe left to skip for now
        </p>
      </div>
